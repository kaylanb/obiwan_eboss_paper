...
\section{\legacypipe\, Measurements of Injected ELGs}
...
\subsection{ELG Number Density and \tractor\, Model Selection}
...
\subsection{Recovered, Contamination, and Loss in the Simulated ELG Sample}

\legacypipe\, modifies the simulated galaxy sample in three ways: true positives, false positives, and false negatives. True positives (recovered ELGs) are simulated ELGs that remain eBOSS ELGs using \legacypipe's measurements for them. False positives (contaminants) are simulated non-ELGs that pass target selection after \legacypipe\, measures them. False negatives (lost ELGs) are simulated ELGs that\legacypipe\, fails to detect (not-detected), fail target selection after \legacypipe\, measures them (fail-ts), or are removed by the {\tt{fracin}} cut (fracin-cut; see Section \ref{sec:biases-systematics}, Fig. \ref{fig:dflux-systematic}).
%There are two types of simulated ELGs in the sample of galaxies that pass eBOSS target selection using \tractor\, catalogues: 

The distributions of recovered ELGs, contaminants, and lost ELGs in the eBOSS color box are shown in Fig. \ref{fig:rec-lost-contam-color} and their \gb, \rband, \zb\, magnitude distributions are shown in Fig. \ref{fig:rec-lost-contam-mag}.
...
Fig. \ref{fig:redshifts-recovered} shows how the input n(z) for eBOSS ELGs is modified by \legacypipe\, and the fraction of contaminants in redshift bins.
...
\subsection{Uncertainties on \tractor\, Measurement of flux, $\rhalf$, and ellipticity (e1, e2)} 

\begin{tabular}{ccc}
      One pass & Two passes & Three passes \\
      % list of figures goes left to right, then down a row, ...
      \includegraphics[width=0.2\textwidth]{num_std_dev_gaussfit_flux_bytype_all_keepwhatputin_fracin_020} &
      \includegraphics[width=0.2\textwidth]{num_std_dev_gaussfit_rhalf_bytype_EXP_submean} &
      \includegraphics[width=0.2\textwidth]{num_std_dev_gaussfit_rhalf_bytype_EXP_submean} 
      \\
      &
      \includegraphics[width=0.2\textwidth]{num_std_dev_gaussfit_rhalf_bytype_DEV_submean} &
      \includegraphics[width=0.2\textwidth]{num_std_dev_gaussfit_rhalf_bytype_DEV_submean} 
      \\
      &
      \includegraphics[width=0.2\textwidth]{num_std_dev_gaussfit_rhalf_bytype_SIMP_submean} &
      \includegraphics[width=0.2\textwidth]{num_std_dev_gaussfit_rhalf_bytype_SIMP_submean}
      \\
\end{tabular}
...
%       \includegraphics[width=0.45\textwidth]{num_std_dev_gaussfit_flux_bytype_all_keepwhatputin_fracin_020}
%     }
%     \wfill
%     \subfloat[EXP\label{fig:rhalf-exp}]{
%       \includegraphics[width=0.45\textwidth]{num_std_dev_gaussfit_rhalf_bytype_EXP_submean}
%     }
%     \hfill
%     \subfloat[DEV\label{fig:rhalf-dev}]{
%       \includegraphics[width=0.45\textwidth]{num_std_dev_gaussfit_rhalf_bytype_DEV_submean.png}
%     }
%      \hfill
%     \subfloat[SIMP\label{fig:rhalf-simp}]{
%       \includegraphics[width=0.45\textwidth]{num_std_dev_gaussfit_rhalf_bytype_SIMP_submean.png}
%     }
%\caption{}
...
\begin{figure}
 ...
 \caption{Number of standard deviations away from truth of \tractor's \gb, \rband, \zb\, flux measurements. There is a systematic offset between \tractor\, and truth which we remove by subtracting the mean (see Section \ref{sec:biases-systematics}). \tractor's \gb, \rband, \zb\, flux errors appear gaussian distributed and are 1.75-2x larger than \tractor ivar implies.}
 \label{fig:redshifts-recovered}
\end{figure}
...
\section{Additional biases and systematics in \legacypipe\, and \obiwan}
...
\begin{figure}
     \subfloat[\label{fig:fracin-2dhist}]{
       \includegraphics[width=0.45\columnwidth]{fracin_vs_numstddev_2dhist}
     }
     \hfill
