...
Because the SGC \gb \, mag PDF in Fig \ref{fig:ngc-incomplete} is a step function for $g > 22.825$, a Gaussian Mixture model for the brightness, shape, and redshift distribution of eBOSS ELGs will be inaccurate at the faint \gb \, mag, where most of the ELGs reside. To model the full distribution of ELG properties we bootstrap sample from our 78,000 EXP and 7,500 DEV samples.

\subsection{ELGs Almost Selected By eBOSS}
\label{sec:eboss-almost-targets}

In addition to simulating eBOSS ELGs, we want to inject ELG-like galaxies outside the eBOSS selection boundaries to test contamination; however, the EXP and DEV samples from Section \ref{sec:eboss-targets} do not go outside the eBOSS ELG selection boundaries. We find that the a DR3-DEEP2 catalogue, using 1\arcsec matching, reproduces the brightness, shape, and redshift distribution of the EXP and DEV samples from Section \ref{sec:eboss-targets}. Fig. \ref{fig:dr3dp2-reproduces} shows that the PDFs of \gb, \rband, \zb, redshift, and $\rhalf$ for the EXP sample from Section \ref{sec:eboss-targets} and the DR3-DEEP2 catalogue, cut to EXP sources, the SGC, and eBOSS ELG targets, agree very well. Fig. \ref{fig:dr3dp2-redshift} shows that the \gb, \rband, \zb, and $\rhalf$ dependence on redshift is also very similar for both samples. A similar level of agreement occurs for DEV galaxies.

\begin{figure}
 \includegraphics[width=\columnwidth]{dr3dp2_reproduces}
 \caption{PDFs of \gb, \rband, \zb mag (top) and redshift and $\rhalf$ (bottom) for EXP galaxies that are eBOSS ELG targets in the DR3-DEEP2 (blue) and Section \ref{sec:eboss-targets} (red) samples. The DR3-DEEP2 sample reproduces the brightness, shape, and redshift distributions from Section \ref{sec:eboss-targets}.}
 \label{fig:dr3dp2-reproduces}
\end{figure}

\begin{figure}
 \includegraphics[width=\columnwidth]{dr3dp2_redshift_trends}
 \caption{Redshift dependence of \gb, \rband, \zb, and $\rhalf$. The DR3-DEEP2 (red) and Section \ref{sec:eboss-targets} (blue) samples agree very well. SWAP COLORS SO BLUE IS DR3-DP2 LIKE PREV FIG}
 \label{fig:dr3dp2-redshift}
\end{figure}

The DR3-DEEP2 catalogue contains galaxies outside the eBOSS target selection boundaries. Because these galaxies are representative of eBOSS ELGs (Figs. \ref{fig:dr3dp2-reproduces} and \ref{fig:dr3dp2-redshift}), we can sample the properties of ELG-like galaxies outside the eBOSS target selection boundaries. We keep DR3-DEEP2 catalogue SGC galaxies within 0.2 mag of the eBOSSS target selection boundaries. This yields a small sample if 1,000 EXP and 100 DEV, but it is better than nothing. 
...
Fig. \ref{fig:dr3dp2-vs-eboss} compares to the eBOSS and DR3-DEEP2 samples of EXP and DEV galaxies, respectively.  
...
\caption{PDFs of \gb, \rband, \zb mag, redshift, and $\rhalf$ for galaxies from the Section \ref{sec:eboss-targets} (red) and DR3-DEEP2 (blue) samples. (Top) EXP galaxies. (Bottom) DEV galaxies. The DR3-DEEP2 sample includes galaxies within 0.2 mag of the eBOSS ELG target selection boundaries, so it is brighter and fainter than the Section \ref{sec:eboss-targets} sample. The blue DEV PDFs are noisy because the DEV DR3-DEEP2 sample is so small (100 galaxies).}
