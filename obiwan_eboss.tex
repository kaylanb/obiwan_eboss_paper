...
eBOSS ELG targets have clean DECaLS photometry, are separated from bright stars/objects based on  bright star masks, have sufficient \gb-flux to be [O II] emitters and star forming galaxies, and have \grcolor\, and \rzcolor\, color associated with galaxies in the desired redshift range of 0.5 - 2 \citep{anand17}. The imaging footprint is split into two regions (the blue boxes in Fig. \ref{fig:ccds-eboss}), about 620 deg$^2$ in the South Galactic Cap (SGC) and 600 deg$^2$ in the North Galactic Cap (NGC). ELGs in the SGC are selected using the following criteria,

\noindent The NGC criteria is the same except for,
