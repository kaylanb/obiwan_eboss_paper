...
The covariance of any statistic, such as the mean $\bar{x}$, is
\begin{align}
C_{jk}(x_i,x_j) = \frac{1}{N}\sum_i^k (x_i^k - \bar{x}_i) (x_j^k - \bar{x}_j),
\end{align}
\noindent where N is the number of subsamples. For Jackknife subsamples, the covariance ($C_{jack}$) is
\begin{align}
C_{\rm{Jack}} = (N-1) \, C_{jk}(x_i,x_j),
\end{align}
\noindent because each subsample contains $(N-1) / N$ of the sample by area.
 
The statistic we are estimating is 
\begin{align}
w(\theta) = 
\end{align}
