...
The correlation function is a clustering statistic that measures the clustering of galaxies, relative to a unclustered random distribution, for a range of galaxy-galaxy separations. The two point correlation function (2PCF, $\xi(r)$) uses the 3D positions (RA, Dec, and redshift) of the galaxies, while the angular correlation function (ACF, $w(\theta)$) uses the 2D positions (RA, Dec). We will use the ACF to gauge the scientific impact of \obiwan\, on eBOSS science.
...
\end{align}
\noindent where n is the surface density of galaxies. The ACF is how much more (or less) likely we are to find a galaxy than if the galaxies were unclustered and randomly distributed. The minimum variance estimator for the ACF is \citep{landy93},  
\begin{align}
...
%The randoms are a sample of random locations on a unit sphere that are then modified by the same procedure used to select the galaxy sample.    

We will compute the ACF using Jackknife sampling, where 
...
clustering measurement is made using each subsample 

jackknife method overestimates the variance by up to 25\% \cite{corrfuncErrors}.
