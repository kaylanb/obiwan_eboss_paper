...
\subsection{Brightness, shape, and redshift for eBOSS ELG targets}
...
\end{itemize}
DEV galaxies are systematically larger and about 1 mag brighter than EXP in all bands (see Fig. \ref{fig:dev-brighter}), so we split the above sample into separate DEV and EXP samples. Fig. \ref{fig:ngc-incomplete} also shows that the NGC region is not complete for the \gb \,mag of the eBOSS ELGs, so we further trim the samples to just the SGC region. This results in about 7,500 DEV and 78,000 EXP galaxies.  
...
For eBOSS, we find that the redshift, brightness, and shape distributions of eBOSS SGC targets with eBOSS spectra is reproduced by the DR3-DEEP2 catalogue cut to the eBOSS ELG TS box. We use the SGC data, because it is deeper than NGC and complete to about g =  22.8 (NGC complete to about g = 22.3). SGC is also 70\% of the data set, so we drop the NGC data. We are interested in redshift, brightness, and shape outside of the eBOSS TS box too, so the DR3-DEEP2 sample cut to 0.2 mag padding tells us that.
