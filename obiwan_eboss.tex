...
We select real galaxies (DD) from the DR3-plus catalogues\footnote{\url{https://data.sdss.org/sas/ebosswork/eboss/sandbox/lss/catalogs/versions/1_1/eBOSS_ELG_full_ALL_v1_1.dat.fits}}, and randoms (RR) from the \obiwan\, catalogues, by applying the NGC criteria for eBOSS ELGS (Section \ref{sec:eboss-ts}) and these additional cuts to remove sources coming from low depth imaging,
\begin{itemize}
\item \verb|psfdepth_g| $> 62.797$
\item \verb|psfdepth_r| $> 30.057$ 
\item \verb|psfdepth_z| $> 11.0$
\end{itemize}
We apply the veto masks
%\footprint{\url{https://trac.sdss.org/wiki/eBOSS/QGC/ELG/ELG_data\#VetoMasks}} 
from \cite{anand17} to the DD and RR samples. To match the DD and RR footprints, we restrict all datasets to the eBOSS 23 footprint 
%\footnote{\url{https://trac.sdss.org/wiki/eBOSS/QGC/ELG/ELG_data\#FootprintMasks}} 
and require $\text{Dec} > 14.05$. We use the \cite{landy93} estimator (Eqn. \ref{eqn:landy93}) to compute the ACF. The angular separation between two RA, Dec points is
\begin{align}
\cos(\theta_1) \cos(\theta_2) \cos(\phi_1-\phi_2) + \sin(\theta_1)\sin(\theta_2),
\end{align}
\noindent where $\theta = (-\text{Dec}+90.) \pi/180$ and $\phi = \text{RA} \pi/180$. 

Fig. \ref{fig:cf} shows the ACF with and without \obiwan\, randoms. The CFHT-BOSS-DEEP2 correlation function (see Section \ref{sec:data-corrfunc}) is shown for reference, and it agrees with \obiwan. We compute $w(\theta)$ at 27 evenly spaced logarithmic $\theta$ bins, centered between 10$^{-2}$ and 5 deg. The mean redshift of the eBOSS ELG n(z) is $z \sim 0.8$. This corresponds to $\sim 20\,h^{-1}\,$ Mpc per degree, so our $\theta$ bins span $\sim 0.2 - 100 \,h^{-1}\,$ Mpc.
...
\caption{Angular correlation function with and without \obiwan. Blue is the \obiwan\, result and green is for uniform randoms. Orange is the CFHT-BOSS-DEEP2 correlation function, which agrees well with \obiwan. (Top) $w(\theta)$ versus $\theta$. (Middle) $\theta \times w(\theta)$ versus $\theta$. (Bottom) Zoom in on Middle plot. We use 27 evenly spaced logarithmic $\theta$ bins, centered between 10$^{-2}$ and 5 deg. The mean redshift of the eBOSS ELG n(z) is $z \sim 0.8$. This corresponds to $\sim 20\,h^{-1}\,$ Mpc per degree, so our $\theta$ bins span $\sim 0.2 - 100 \,h^{-1}\,$ Mpc.}
\label{fig:cf}
\end{figure}

Each data point in Fig. \ref{fig:cf} is the ACF of the full footprint ($\bar{w(\theta)}$). The error bars come from Jackknife sampling, as discussed in Section \ref{sec:methods-acf}, using $N_{sub} = 20$ equal area subsamples (Healpix pixels). There are thousands of galaxy-galaxy pairs per subsample so we can ignore the chance fluctuation contribution to $\sigma_{\rm{w(\theta)}}$.  
...
the per-brick weight maps could be used to create mock data sets using N-body simulations for the evolution of dark matter clustering in the universe. The variance on the angular correlation could be estimated from such mocks, instead of with the Jackknife method as we did in this paper.
