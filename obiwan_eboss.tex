...
\end{itemize}
DEV galaxies are systematically larger and about 1 mag brighter than EXP in all bands (see Fig. \ref{fig:dev-brighter}), so we split the above sample into separate DEV and EXP samples. Fig. \ref{fig:ngc-incomplete} also shows that the NGC region is not complete for the \gb \,mag of the eBOSS ELGs, so we throw out all the NGC galaxies (about 30\% of the sample). This yields 77,525 EXP and 7,439 DEV galaxies. We refer to these as our eBOSS sample.
...
In addition to simulating eBOSS ELGs, we want to test contamination by injecting ELGs just outside of the eBOSS selection boundaries; however, the eBOSS sample does not extend beyond the selection boundaries. We find that the a DR3-DEEP2 catalogue, using 1\arcsec matching, reproduces the brightness, shape, and redshift distributions of the eBOSS sample. Fig. \ref{fig:dr3dp2-reproduces} shows the PDFs of \gb, \rband, \zb, redshift, and $\rhalf$ for EXP galaxies, and that the DR3-DEEP2 galaxies (cut to type EXP galaxies, the SGC footprint, and eBOSS ELG targets) agree very well with the eBOSS sample. Fig. \ref{fig:dr3dp2-redshift} shows that for EXP galaxies, \gb, \rband, \zb, and $\rhalf$ depend on redshift in the same way for the DR3-DEEP2 galaxies and the eBOSS sample. A similar level of agreement occurs for DEV galaxies.
...
Because DR3-DEEP2 galaxies are representative of eBOSS ELGs and extend beyond the eBOSS selection boundaries, we can construct a sample of ELG-like galaxies to test contamination into the eBOSS selected sample. We choose to keep DR3-DEEP2 SGC galaxies within a padding of 0.2 mag of the eBOSSS selection boundaries, yielding a sample of 1,000 EXP and 100 DEV galaxies. Burleigh \& Moustakas 2018, in prep show that \tractor\, flux measurements are accurate to 0.1 - 0.2 mag for \gb $\sim 22.9$ galaxies, so a padding of 0.2 mag should capture most contaminants due to \tractor\, measurement errors. 

\subsection{Simulating eBOSS ELGs with the correct n(z)}

Given our four representative samples for eBOSS ELGs: the eBOSS sample (77,525 EXP and 7,439 DEV galaxies) and the DR3-DEEP2 sample (1,064 EXP and 85 DEV galaxies), we want to bootstrap sample them according to the eBOSS n(z). 

Fig. \ref{fig:dr3dp2-vs-eboss} shows how the EXP samples compare. There are many more data points in the SGC EXP sample, but the DR3DP2 EXP sample extends to fainter and brighter \gb, \rband, and \zb \, mag ELG-like galaxies.
