...
How the angular correlation function behaves for eBOSS-like ELGs has been determined by previous studies. We compare this known result to our angular correlation function in Section \ref{sec:results-cf}. \cite{corrfuncEboss} computed the angular correlation function for an ELG galaxy sample selected using $\sim 50$ deg$^2$ of $ugri$-photometry from the Canada-France-Hawaii Telescope Legacy Survey (CFHT-LS) and (1\arcsec matched) spectroscopy from BOSS DR12 and DEEP2. Their sample is located in the W3 field, has 500 ELGs per deg$^2$, a redshift range of $0.6 < z < 1$. We will refer to this as the CFHT-BOSS-DEEP2 angular correlation function.
...
For computing the angular correlation function: \citep{corrfuncHamilton, corrfuncErrors, corrfuncIC}.

\begin{itemize}
\item N$_{sub}$ subsamples of equal area. This is k-fold cross validation with $k=N_{sub}$ but with spatially not randomly chosen subsamples. Too many subsamples (i.e. subsamples with too small an area) creates dependent samples, while too few subsamples does not appreciably reduce (by $1/\sqrt{N_{sub}}$ ) the uncertainty on the \red{variance/average of counts?}. The spatial extent of each subsample also determines the maximum 
\item clustering measurement is made using each subsample 
\item jackknife method overestimates the variance by up to 25\% \cite{corrfuncErrors}.
\item (for discussion) the per-brick weight maps could be used to create mock data sets using N-body simulations for the evolution of dark matter clustering in the universe. The variance on the angular correlation could be estimated from such mocks, instead of with the Jackknife method as we did in this paper.
\end{itemize}
 
The joint probability of finding two objects of interest in solid angle $d\Omega_1$ and $d\Omega_2$ separated by angle $\theta$ is given by
\begin{align}
dP(\theta)= n^2 [1+w(\theta) ] d\Omega_1 d\Omega_2
\end{align}
 
Our statistic of interest is the angular correlation function. We use the \cite{landy93} minimum variance estimator,  
\begin{align}
...
\end{align}
which is also an unbiased estimator for the two-point correlation function \citep{corrfuncIC}. $DD$ is the number of real galaxy-real galaxy pairs with separation between $\theta$ and $\theta + \Delta \theta$, $RR$ is the number of random-random pairs, and $N_D$ and $N_R$ are the total number of real galaxies and randoms in the data set, respectively.   

Chance fluctuations (aka Poisson noise, shot noise, or cosmic variance) in $DD(\theta)$ and $RR(\theta)$ also contribute to the variance of our $w(\theta)$ measurements. Because $RR(\theta) / DD(\theta) \approx  X$, the $1/\sqrt{DD({\theta})}$ term dominates the chance fluctuation contribution, and the variance in our $w(\theta)$ estimates due to chance fluctuation is \citep{corrfuncIC}
\begin{align}
\sigma_{\rm{w(\theta), Chance}}^2 = \frac{1}{DD(\theta)}[1+w(\theta) ]^2.
\end{align}

Our uncertainties assume that the central limit theorem applies, i.e. our 20 subsamples are sufficiently many so that our $w(\theta)$ estimates are Gaussian distributed \cite{corrfuncHamilton}. $N=50$ leads to $\sim 10$\% errors on the variance \red{HOW SHOW THIS?}. For our Jackknife subsamples, assuming that our$w(\theta)$ estimates are Gaussian distributed (i.e. 20 subsamples is sufficiently many for the Central Limit Theorem to apply), the variance of the distribution of our $w(\theta)$ estimates is
\begin{align}
\sigma_{\rm{w(\theta), Jack}}^2 = \frac{N-1}{N}\sum_i^N (w_i(\theta) - \bar{w(\theta)})^2,
\end{align}
\noindent where N is the number of subsamples, and $(N-1) / N$ is the fraction of the total sample, by area, of each subsample. 
...
We select real galaxies from the DR3-plus catalogues, and randoms from the \obiwan catalogues, by applying the NGC criteria for eBOSS ELGS (Section \ref{sec:eboss-ts}) and these additional cuts to remove sources coming from low depth imaging,
\begin{itemize}
...
We compute the angular distance between two RA, Dec point as
\begin{align}
\cos(\theta_1) \cos(\theta_2) \cos(\phi_1-\phi_2) + \sin(\theta_1)\sin(\theta_2),
\end{align}
\noindent where $\theta = (-\text{Dec}+90.) \pi/180$ and $\phi = \text{RA} \pi/180$.

The error bars in Fig. \ref{} are the standard deviations of the Jackknife subsamples (Eqn. \ref{eqn:varjack}) because $\sigma_{\rm{w(\theta), Chance}} \sim X \ll \sigma_{\rm{w(\theta), Jack}}$.

Green is the CFHT-BOSS-DEEP2 angular correlation function (see Section \ref{sec:data-corrfunc}).   

The mean redshift of eBOSS n(z) is $z \sim 0.8$, which corresponds to $\sim 20\,h^{-1}\,$ Mpc per degree. The range in Fig. \ref{} is roughly $X - X \,h^{-1}\,$ Mpc.

\subsection{Anymask}

\red{INSERT FRACTION ANYMASK / ALLMASK heatmap}
