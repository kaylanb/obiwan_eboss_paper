...
We inject 2400 galaxies / deg$^2$ in both footprints, of which X\% are eBOSS ELG targets. The eBOSS ELG target densities are 200 and 240 targets / deg$^2$ in the NGC and SGC, respectively, so the density of simulated ELGs is at least X times higher than the target density for real ELGs.
...
\end{align}
\noindent where $\psi = (-\text{Dec}+90) \,\pi/180$ and $\phi = \text{RA} \times \pi/180$. We compute the ACF using the \cite{landy93} estimator (see Eqn. \ref{eqn:landy93}) and plot the results in Fig. \ref{fig:cf}. 

Fig. \ref{fig:cf} compares the ACFs for \obiwan-randoms (the new method proposed in this paper) and uniform-randoms (see Section \ref{sec:methods-obiwan}) to the CFHT-BOSS-DEEP2 correlation function (see Section \ref{sec:data-corrfunc}). The CFHT-BOSS-DEEP2 ACF agrees well with the result using \obiwan-randoms. We compute $w(\theta)$ at 27 evenly spaced logarithmic $\theta$ bins, centered between 10$^{-2}$ and 5 deg. The mean redshift of the eBOSS ELG n(z) is $z \sim 0.8$. This corresponds to $\sim 27$ Mpc per degree, so our $\theta$ bins span $\sim 0.27 - 135$ Mpc.
...
Although computing the angular correlation function with \obiwan\, does not require weight-maps, \obiwan\, can produce various weight maps for use by future map-based studies. The maps could also be used to create mock data sets from N-body simulations of the evolution of dark matter clustering in the universe. For example, the variance of our angular correlation function measurement could be estimated from the mocks, instead of with Jackknife sampling. 

We limit ourselves to three weight maps that might be of interest to future eBOSS ELG studies: 
\begin{itemize}
...
\cprotect\caption{Weight-maps of imaging systematics that may be useful for future studies that remove imaging systematics with map-based methods. (Top) The fraction of all injected sources that \legacypipe\, detects and measures. (Middle) The fraction of true NGC eBOSS ELGs that \legacypipe\, detects and measures and that have \tractor\, measurements that pass NGC eBOSS ELG target selection. (Bottom) Using \tractor\, measurements, the ratio of the number of sources that pass NGC eBOSS ELG target selection using \verb|allmask_grz = 0| to the number of when using \verb|anymask_grz = 0|. }
\label{fig:weight-maps}
\end{figure}

In the Recovered map, higher fractions are generally due to more CCDs (see Fig. \ref{fig:ccds-eboss}); however, in Recovered NGC-ELGs, fewer eBOSS NGC ELGs are selected based on \tractor\, measurements in regions with many and few CCDs, respectively. The Anymask-Allmask-Ratio map shows that the \verb|anymask_grz = 0| cut removes more sources when there are more CCDs (i.e. deeper imaging). It can be up to a 40\% effect but is less than a $10$\% effect on average.
