...
Removing imaging systematics is critical for measuring unbiased clustering statistics, such as the angular correlation function. This has primarily been done for the SDSS, WiggleZ, \red{others?} using map-based methods. Starting from a large scale structure (LSS) catalogue containing selected galaxy positions and properties, the sky is subdivided into equal-area pixels using a pixelization scheme such as \healpix\, \citep{healpix}. The number of galaxies and statistics (usually the average) for imaging meta-data (e.g. seeing, sky brightness, exposure time, etc.) and foregrounds (e.g. galactic extinction) are computed per-pixel. Statistics having sufficiently large cross correlation with the number of galaxies are removed by the given method \red{is this template subtraction only or also true to model projection?}. The defining feature of map-based methods is the construction of a weight map, which mimics how the angular selection function up- or down-samples the true distribution of galaxies to yield the observed LSS catalogue.
...
Both methods do the following: each discrete (pixelized) map is described as a summation of basis functions (usually spherical harmonics). Computing a correlation function or power spectrum in Fourier space ``projects" the data onto the basis function's modes. The data is the superposition of modes from systematic maps and the underlying cosmological signal. The ``template subtraction" method models each systematic map and subtracts it from each mode in Fourier space or up- and down-weights the pixel values in configuration space. The ``mode projection" method treats the systematic maps as adding noise to each mode in Fourier space or pixels in configuration space, so that values in the data covariance matrix are increased for modes where each systematic map is large. The mode project method robustly models the impact of the linear combination of the systematics. It does not include non-linear affects from the systematics. Neither method corrects for systematics that are not known apriori.

Drawbacks of the map-based methods include the following. A substantial fraction of the data ($10-20$\% \citep{elvinpoole}, \red{others?}) is removed because it contains bright stars, bad seeing, etc. and the number of galaxies deviate significantly from the mean. The software to create a LSS catalogue is complex and can introduce algorithmic biases and systematics into the LSS catalogue. Future and ongoing galaxy surveys, such as the Legacy Surveys, will select galaxies from a join analysis of images from multiple telescopes, multi- and same-band dithers will be separated by month to year time baselines, and the CCD detectors are large and will have similar angular size (about 1 deg) to the cosmological signal of interest (e.g. the BAO signal is X deg at a redshift of X). It is unclear whether map-based methods are capable of removing imaging systematics, especially due to the latter, at a level sufficient for a Stage-IV dark energy science. 
...
For example, \cite{elvinpoole} needed more than 19 maps to remove systematics from DECam data.

The Extended Baryon Oscillation Spectroscopic Survey (eBOSS) \red{cite} targetted emission line galaxies (ELGs) using DECaLS data and obtained spectra for them. The Dark Energy Spectroscopic Instrument (DESI) \red{cite} will obtain spectra for about 30M galaxies and all targets will come from DECaLS, MzLS, BASS, and WISE images. 

\obiwan\, operates on the \legacypipe/\tractor pipeline, so our goal is to use the angular correlation function for eBOSS ELGs to estimate the impact of \obiwan\, on the primary science objectives of eBOSS. This is also a preparatory step towards future analysis of the DESI ELG sample, since DESI is significantly more complicated than eBOSS. All data products are available at NERSC (see Section \ref{sec:data-products}).

We present a new method for removing imaging systematics from Legacy Survey data that does not require a weight map, does not assume apriori knowledge of the systematics of interest, and that removes biases and systematics in the software pipeline that produced the LSS catalogue. We use the \obiwan\, code  (Burleigh \& Moustakas 2018, in prep) to perform Monte-Carlo simulations of injecting simulated ELG galaxies into the DECam images used for eBOSS ELG target selection \citep{anand17} and forward-modeling the the simulated sources with the \tractor. \obiwan\, is a non-parametric method that corrects for the full covariance of imaging systematics. This is a crucial test of the impact of \obiwan\, on future BAO measurements.

For eBOSS this is the only way to model the anymask == 0 cut.

Useful references \cite{peebles1980, apmSurvey}
...
We use the joint tables of \tractor\, catalogues and eboss21, 22, 23 spectra, 
\begin{itemize}
...
\end{itemize}
to construct a sample of eBOSS ELG targets having a redshift, shape, and \gb, \rband, \zb flux. 
...
We use \obiwan\, to inject simulated galaxies into the DECam CCDs\footnote{\url{http://portal.nersc.gov/project/desi/users/kburleigh/obiwan/legacysurveydir_ebossdr3}} that were used to create the DR3-era \tractor\, catalogues and to select the eBOSS ELG targets \citep{anand17}. Fig. \ref{fig:ccds-eboss} shows these CCDs. We limit our \obiwan\, runs to the blue box. 
