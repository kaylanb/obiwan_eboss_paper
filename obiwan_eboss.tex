...
INSERT discussion of Baryonic Acoustic Oscillations. The Sloan Digital Sky Survey (SDSS) \cite{sdssYork}, X, X, and X galaxy surveys were carried out to detect the BAO signal. SDSS first detected the BAO signal at redshift X at signal to noise (S/N) of X \cite{eisenstein05}. The most recent detections are from the X survey for redshifts X, X, and X at S/N of X, X, and X, respectively \red{cite papers} and the X survey for redshifts X, X, and X at S/N of X, X, and X, respectively \red{cite papers}. Measuring the BAO signal and using priors on X, X, X cosmological parameters from Planck \red{planck papers}, places constraints on w0 and w1, the cosmological parameters that describe the rate of expansion of the universe. The Dark Energy Spectroscopic Instrument (DESI), is a robotically actuated 5,000-fiber spectrograph that will obtain 30M spectra of galaxies and quasars to make a Stage-IV measurement of dark energy (e.g. measure w0 and w1 to sub-percent accuracy)\citep{desiScience, desiInstrument}. DESI is a five-year survey with first light in early 2019. The BAO signal will be measured at X redshifts at S/N of X. 

Galaxy Surveys
sdss \cite{sdssYork}
sdss7 \citep{sdssDR7}
Two-degree-Field Galaxy Redshift Survey (2dF) \citep{2dF}
WiggleZ Dark Energy Survey (WiggleZ) \citep{wigglezSurvey}

Other surveys
Dark Energy Survey (DES) \citep{des}
\citep{desiScience, desiInstrument}
Dust map \cite{schlegel98}
Tractor \cite{lang16}

bao detection and physics
robustness of bao \cite{eisensteinRobustnessBao}
reconstruction and nP = 1 \cite{eisensteinReconstructionNp}
cosmic sound \cite{eisensteinCosmicsound}
first detection \cite{eisenstein05}
wiggle z low redshift \cite{wigglezLowz} and high redshift \cite{wigglezHighz} detections
more physics \cite{eisensteinAndHu, eisenstein05, eisensteinCosmicsound, seo07, reid12, beutler14, beutler16}

Removing imaging systematics, such as stellar density, seeing, and galactic extinction, from galaxy survey data is critical to making a high signal to noise (S/N) detection and unbiased measurement the Baryonic Acoustic Oscillation (BAO) signal. The Sloan Digital Sky Survey (SDSS; \red{cite}), \red{insert others} \red{cite}, removed imaging systematics by cross correlating each systematic with the number of galaxies detected, keeping the most correlated systematics, and fitting a (usually) linear or quadratic model to the each systematic's correlation with galaxy counts. The model for each systematic predicts how much to up- or down-sample the number of galaxies in a given part of the sky. This method assumes apriori knowledge of the potential imaging systematics and that they are independent of one other. These assumptions will likely break down in the next generation of galaxy surveys, such as the Legacy Surveys (aka DECaLS, MzLS, and BASS) \red{CITE OVERVIEW PAPER}. The Legacy Surveys take images with three telescopes and leverage existing imaging from Wide-field Infrared Survey Explorer (WISE) \red{cite} to detect galaxies. Multi- and same-band images of the same part of the sky have time baselines of months to years, not minutes. The Extended Baryon Oscillation Spectroscopic Survey (eBOSS) \red{cite} targetted emission line galaxies (ELGs) using DECaLS data and obtained spectra for them. The Dark Energy Spectroscopic Instrument (DESI) \red{cite} will obtain spectra for about 30M galaxies and all targets will come from DECaLS, MzLS, BASS, and WISE images. 

The complexities of the Legacy Surveys have already revealed shortcomings in the current method for removing imaging systematics. For example, \red{cite Elvin-Poole paper} fit models to multiple imaging systematics but this requires a procedure for dealing with the many ($> 20$) epoch images for a given part of the sky. \red{fix previous sentence, add other studies}.

We present a new method for removing imaging systematics from the eBOSS ELG sample \cite{anand17} using the \obiwan\, code, which is a Forward-Modeling and Monte-Carlo Simulation tool for injecting simulated galaxy targets into Legacy Surveys data at the single exposure level (Burleigh \& Moustakas 2018, in prep). \obiwan\, is a non-parametric method that corrects for the full covariance of imaging systematics. This is a crucial test of the impact of \obiwan\, on future BAO measurements. Our goal is to use the angular correlation function for eBOSS ELGs to estimate the impact of \obiwan\, on the primary science objectives of eBOSS and how much it improves over the previous method described above. This is also a preparatory step towards future analysis of the DESI ELG sample, since DESI is significantly more complicated than eBOSS. All data products are available at NERSC (see Section \ref{sec:data-products}).
