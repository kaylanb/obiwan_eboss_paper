...
\caption{The eBOSS NGC and SGC footprints (blue boxes) and the DECaLS CCDs from the \tractor\, catalogues that CITE ANAND PAPER used to select ELG targets.}
\label{fig:ccds-eboss}
\end{figure}

\subsection{Simulated Galaxy Sample}

\dev are 9\% of the eBOSS sample and they fill in parameter space enough for use to model. For eBOSS, we find that the redshift, brightness, and shape distributions of eBOSS SGC targets with eBOSS spectra is reproduced by the DR3-DEEP2 catalogue cut to the eBOSS ELG TS box. We use the SGC data, because it is deeper than NGC and complete to about g =  22.8 (NGC complete to about g = 22.3). SGC is also 70\% of the data set, so we drop the NGC data. We are interested in redshift, brightness, and shape outside of the eBOSS TS box too, so the DR3-DEEP2 sample cut to 0.2 mag padding tells us that.
...
\begin{figure}
 \includegraphics[width=\columnwidth]{grz_hist}
 \caption{}
...
\begin{figure}
 \includegraphics[width=\columnwidth]{delta_dec_vs_delta_ra}
 \caption{}
...
\caption{Number density of simulated galaxies in the NGC and SGC footprints.}
...
\caption{Fraction of injected galaxies recovered by \legacypipe\, (i.e. detected and modeled) in the NGC and SGC footprints.}
